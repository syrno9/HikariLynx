...
boardManagement.init = function() {
  ...
  var messageLengthLabel = document.getElementById('messageLengthLabel');

  if (messageLengthLabel) {
    boardManagement.messageLimit = +messageLengthLabel.innerText;
  }

  boardManagement.volunteerCellTemplate = api.getTemplate("management-volunteer-template");
  ...
  if (document.getElementById('ownerControlDiv')) {
    ...
    Array.from( volunteerDiv.children ).forEach( (child) => {
      if (child.nodeName !== "form")
        return 
      boardManagement.processVolunteerCell(child);
    });
  }
...
boardManagement.setIndicatorForRequest = function(obtainedSpoiler) {
  ...
  if (haveSpoiler !== obtainedSpoiler) {
    ...
    if (obtainedSpoiler) {
      ...
      indicatorText.innerText = 'There is a custom spoiler saved for this board';
...
boardManagement.setVolunteersDiv = function(volunteers) {
  ...
  for (var i = 0; i < volunteers.length; i++) {
    ...
    boardManagement.volunteerCellTemplate.cloneInto(cell);

    cell.getElementsByClassName('userIdentifier')[0].setAttribute('value',
        volunteers[i]);

    cell.getElementsByClassName('userLabel')[0].innerText = volunteers[i];
