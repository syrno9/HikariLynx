...
filterManagement.init = function() {
  ...
  filterManagement.maxLength = +document.getElementById('maxLengthLabel').innerText;
...
filterManagement.seekCell = function(filter) {
  ...
  for (var i = 0; i < candidates.length; i++) {
    ...
    if (candidates[i].innerText === filter) {
      return candidates[i];
    }
            ...
            if (status === 'ok') {
              ...
              if (existing) {
                existing.parentNode.parentNode
                    .getElementsByClassName('labelReplacement')[0].innerText = typedReplacement;
